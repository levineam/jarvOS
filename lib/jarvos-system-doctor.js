...
const EVIDENCE_ATTENTION_REASON_CLASSES = ['module-invalid', 'module-stale', 'module-untrusted'];

function moduleState(state, reasonClass) {
  if (EVIDENCE_ATTENTION_REASON_CLASSES.includes(reasonClass)) return 'warning';
...
function declaredSystemFactsVersion(modules = []) {
  const system = modules.find((module) => module.id === 'system');
  if (!system || !Array.isArray(system.components) || typeof system.factsVersion !== 'string') return null;
  return system.factsVersion;
}
...
function buildSystemDoctorReceipt(report) {
  ...
  return {
    ...
    status,
    factsVersion: declaredSystemFactsVersion(report.modules),
    components,
  };
}
