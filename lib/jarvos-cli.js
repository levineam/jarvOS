...
function parseCodexJarvosMcp(content) {
  ...
  for (const line of lines.slice(start + 1)) {
    ...
    if (!match) continue;
    try { entry[match[1]] = JSON.parse(match[2].replace(/'([^']*)'/g, (_, value) => JSON.stringify(value))); } catch { entry[match[1]] = null; }
  }
  return entry;
}
...
function readJarvosMcpRegistrations(options = {}) {
  ...
  for (const source of jarvosMcpRegistrationSources(options)) {
    let content;
    try { content = fs.readFileSync(source.file, 'utf8'); } catch { continue; }
    let entry = null;
    try {
