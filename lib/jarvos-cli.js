...
const COMMANDS = new Set(['help', 'init', 'doctor', 'security']);
...
function runSecurity(argv = [], env = process.env, spawn = spawnSync) {
  const child = spawn(process.execPath, [path.join(ROOT, 'scripts', 'jarvos-security.js'), ...argv], {
    cwd: ROOT,
    env,
    stdio: 'inherit',
  });
  if (child.error) {
    process.stderr.write(`jarvos security failed: ${child.error.message}\n`);
    return 1;
  }
...
function runCli(argv = process.argv.slice(2), env = process.env, invokedAs = process.argv[1]) {
  ...
  if (parsed.command === 'help'
    || parsed.help && !['doctor', 'init', 'security'].includes(parsed.command)) {
  ...
  if (parsed.command === 'security') {
    return runSecurity(normalizedArgv.slice(1), env);
  }
