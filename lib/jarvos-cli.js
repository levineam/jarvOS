...
function runDoctor(options = {}) {
  ...
  const modules = loadHealthModules({ workspace, now: options.now || new Date() }).modules;
  const moduleBlocking = modules.some((module) => ['repair needed', 'needs your attention'].includes(module.state));
...
Usage:
  ...
  jarvos coding accept --registry /absolute/registry.json --repository-id ID --run-id ID --revision DIGEST [--json]
...
function parseCodingArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    const match = value.match(/^--(registry|repository-id|repository-json|run-id|revision)=(.*)$/);
    if (match) { result.options[match[1]] = match[2]; continue; }
    if (['--registry', '--repository-id', '--repository-json', '--run-id', '--revision'].includes(value)) {
...
function runCoding(argv = []) {
  try {
    ...
    if (area === 'repository') {
      ...
      receipt = recordOwnerAction({ registryPath: options.registry, repositoryId: options['repository-id'], action: 'accept-plan', runId: options['run-id'], revision: options.revision });
