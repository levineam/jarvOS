...
const { attachSystemDoctorReceipt, renderSystemDoctor } = require('./jarvos-system-doctor');
const { inspectCompoundEngineeringProvider, loadRuntimeModeConfig } = require('../modules/jarvos-runtime-kit/src');
const {
...
const COMMANDS = new Set(['help', 'init', 'sync', 'doctor', 'desktop']);
...
function validateConfigShape(config, schema = loadConfigSchema(), options = {}) {
  ...
  const runtimeMode = loadRuntimeModeConfig(config);
  if (!runtimeMode.ok) {
...
// Fresh installs write an explicit, declaration-only `none` mode. Unlike
// legacy configs that merely omit runtimeMode, this is affirmative evidence
// that no resident runtime has been selected and its configured vault has not
// been activated. Keep old and active-mode installations on the strict vault
// health path.
function isExplicitDormantRuntimeMode(config) {
  const runtimeMode = loadRuntimeModeConfig(config);
  return runtimeMode.ok
    && runtimeMode.source === 'configured'
    && runtimeMode.runtimeMode?.mode === 'none';
}
...
function checkVaultPath(configPath, options = {}) {
  try {
    ...
    if (isExplicitDormantRuntimeMode(config) && !fs.existsSync(vaultPath)) {
      return result('vault-path', true, 'vault path', `dormant runtime mode leaves vault unactivated: ${vaultPath}${note}`);
    }
...
// Every harness must reach the jarvOS MCP server through the same entrypoint.
// A managed install rotates immutable runtime stages under a stable selector,
// so a client registered against one stage's own jarvos-mcp.js keeps serving
// that stage after the next promotion -- without the selected runtime's
// Projects bindings -- while clients bound to the stable shim keep working.
// This check reads each harness's persisted registration and fails when a
// target is missing, differs from the managed stable shim, or when harnesses
// disagree about where the server lives.
function jarvosMcpRegistrationSources(options = {}) {
  const env = options.env || process.env;
  const homeDir = options.homeDir || os.homedir();
  const codexHome = env.CODEX_HOME || path.join(homeDir, '.codex');
  return [
    { harness: 'claude-code', format: 'claude-json', file: env.CLAUDE_CODE_CONFIG || path.join(homeDir, '.claude.json') },
    { harness: 'claude-desktop', format: 'claude-json', file: env.CLAUDE_DESKTOP_CONFIG || path.join(homeDir, 'Library', 'Application Support', 'Claude', 'claude_desktop_config.json') },
    { harness: 'codex', format: 'codex-toml', file: env.CODEX_CONFIG || path.join(codexHome, 'config.toml') },
  ];
}

function parseCodexJarvosMcp(content) {
  const lines = content.split(/\r?\n/);
  const start = lines.findIndex((line) => /^\s*\[\s*mcp_servers\.(?:jarvos|"jarvos")\s*\]\s*(?:#.*)?$/.test(line));
  if (start < 0) return null;
  const entry = {};
  for (const line of lines.slice(start + 1)) {
    if (/^\s*\[/.test(line)) break;
    const match = line.match(/^\s*(command|args)\s*=\s*(.+?)\s*$/);
    if (!match) continue;
    // Only the one-line forms Codex writes are understood; anything else is
    // reported as unreadable rather than guessed at.
    entry[match[1]] = JSON.parse(match[2].replace(/'([^']*)'/g, (_, value) => JSON.stringify(value)));
  }
  return entry;
}

function jarvosMcpTarget(entry) {
  if (!entry || typeof entry.command !== 'string' || !entry.command) return null;
  if (entry.args !== undefined && !Array.isArray(entry.args)) return null;
  const args = entry.args || [];
  const interpreter = /^node(?:js)?(?:\.exe)?$/.test(path.basename(entry.command));
  return interpreter && typeof args[0] === 'string' ? args[0] : entry.command;
}

function readJarvosMcpRegistrations(options = {}) {
  const registrations = [];
  for (const source of jarvosMcpRegistrationSources(options)) {
    let content;
    try {
      content = fs.readFileSync(source.file, 'utf8');
    } catch (error) {
      if (error?.code === 'ENOENT') continue;
      registrations.push({ harness: source.harness, file: source.file, target: null, unreadable: true });
      continue;
    }
    let entry = null;
    try {
      entry = source.format === 'codex-toml'
        ? parseCodexJarvosMcp(content)
        : JSON.parse(content)?.mcpServers?.jarvos || null;
    } catch {
      registrations.push({ harness: source.harness, file: source.file, target: null, unreadable: true });
      continue;
    }
    if (!entry) continue;
    registrations.push({ harness: source.harness, file: source.file, target: jarvosMcpTarget(entry) });
  }
  return registrations;
}

function checkMcpRegistration(options = {}) {
  const env = options.env || process.env;
  const registrations = readJarvosMcpRegistrations(options);
  const label = 'jarvOS MCP registrations';
  if (!registrations.length) return result('mcp-registration', true, label, 'no harness registers a jarvOS MCP server');
  const stableRoot = typeof env.JARVOS_STEWARDSHIP_STABLE_ROOT === 'string' && path.isAbsolute(env.JARVOS_STEWARDSHIP_STABLE_ROOT)
    ? env.JARVOS_STEWARDSHIP_STABLE_ROOT
    : null;
  const expected = typeof env.JARVOS_MCP_STABLE_ENTRYPOINT === 'string' && path.isAbsolute(env.JARVOS_MCP_STABLE_ENTRYPOINT)
    ? env.JARVOS_MCP_STABLE_ENTRYPOINT
    : (stableRoot ? path.join(stableRoot, 'jarvos-mcp') : null);
  const problems = [];
  for (const registration of registrations) {
    if (!registration.target) problems.push(`${registration.harness}: registration is unreadable`);
    else if (!fs.existsSync(registration.target)) problems.push(`${registration.harness}: ${registration.target} does not exist`);
    else if (expected && path.resolve(registration.target) !== path.resolve(expected)) problems.push(`${registration.harness}: ${registration.target} is not the stable entrypoint ${expected}`);
  }
  const targets = [...new Set(registrations.filter((registration) => registration.target).map((registration) => path.resolve(registration.target)))];
  if (!expected && targets.length > 1) {
    problems.push(`harnesses disagree: ${registrations.map((registration) => `${registration.harness} -> ${registration.target}`).join('; ')}`);
  }
  if (problems.length) {
    return result('mcp-registration', false, label, `${problems.join('; ')}. Re-run each harness's runtimes/<harness>/setup.sh so every client uses the same stable entrypoint.`);
  }
  return result('mcp-registration', true, label, `${registrations.map((registration) => registration.harness).join(', ')} -> ${targets[0]}`);
}
...
function checkVaultPathStale(configPath, options = {}) {
  ...
  try {
    ...
    if (!fs.existsSync(vaultPath)) {
      if (isExplicitDormantRuntimeMode(config)) {
        return result('vault-path-stale', true, 'vault path freshness', `dormant runtime mode leaves vault unactivated: ${vaultPath}${note}`);
      }
      return result(
        'vault-path-stale',
        false,
...
function runDoctor(options = {}) {
  ...
  const checks = {
    ...
    'mcp-registration': () => checkMcpRegistration(options),
  };
...
Usage:
  ...
  jarvos desktop [install|status] [--workspace path] [--json]
  jarvos help
...
Creates a new jarvOS installation through the public command router. Use it on
a fresh host or for a new harness workspace. When the vault already exists,
pass --use-existing-vault: init creates the starter workspace files and
portable config, validates the existing Notes/, Journal/, and Tags/ folders,
and leaves the vault's existing content untouched.
...
Path selection:
  ...
  --no-desktop      Skip Desktop installation (headless core supports Node 18).

Desktop installs by default and requires Node 22.12+ and a network download.
Launch it explicitly with jarvos desktop --workspace path. Set
JARVOS_NO_DESKTOP=1 for the same headless opt-out in automation.
...
a new/empty workspace and a verified existing vault containing Notes/, Journal/,
and Tags/. Do not use jarvos sync as a replacement for init: sync is a
config-only handoff for a harness workspace that is already installed.
...
function renderSyncHelp() {
  return `jarvos sync — Sync with an existing jarvOS installation (config-only handoff)

Usage:
  jarvos sync --workspace /path/to/workspace --vault /path/to/vault \\
    --name "Your Name" --timezone Area/City --dry-run

Use this command only for a harness workspace that is already installed (for
example, after jarvOS init). It creates or verifies only a portable
jarvos.config.json handoff; it does not create starter workspace files, install
a harness, or initialize vault folders. On a fresh host or new harness
workspace, use jarvos init with --use-existing-vault instead.

In ordinary, uncontended use the command validates Notes/, Journal/, and Tags/
and writes no config contents inside the vault. Use --dry-run first. Applying
the plan creates a new config only; it refuses to replace any different
existing config, and an identical portable config is treated as already
synced. A compatible legacy-shaped config is reported as manual-reconcile:
jarvOS never rewrites an existing config in place. Reconcile that file
manually, or pass --config to a new path. For an existing portable config,
--vault, --name, and --timezone may be omitted: jarvOS reuses those exact
configured values.
...
function runSync(argv = [], env = process.env, { platform = process.platform, constants = fs.constants } = {}) {
  ...
  try {
    ...
    const targetAssessment = assessSharedVaultConfigTarget({ configPath, config, vaultDir, homeDir, env });
...
function runDesktop(parsed, env) {
  if (parsed.help) {
    process.stdout.write('jarvos desktop [install|status] [--workspace path] [--vault path] [--json]\n\nInstall or update explicitly; without a subcommand, launch the selected app.\nStatus is read-only. Workspace defaults to JARVOS_WORKSPACE_PATH or the current directory.\n');
    return 0;
  }
  try {
    const command = parsed.positionals[0] || 'launch';
    if (!['install', 'status', 'launch'].includes(command) || parsed.positionals.length > 1
      || parsed.passthrough.some((arg) => arg.startsWith('-'))
      || Object.keys(parsed.options).some((key) => !['workspace', 'vault', 'json'].includes(key))) {
      throw new Error('Use jarvos desktop [install|status] [--workspace path] [--vault path] [--json]');
    }
    const workspace = parsed.options.workspace || env.JARVOS_WORKSPACE_PATH || process.cwd();
    const options = {
      workspace: path.resolve(expandHome(workspace, env.HOME || os.homedir())),
      vault: parsed.options.vault ? path.resolve(expandHome(parsed.options.vault, env.HOME || os.homedir())) : undefined,
      env,
    };
    const { installDesktop, desktopStatus, launchDesktop } = require('./jarvos-desktop');
    if (command === 'launch') {
      const outcome = launchDesktop(options);
      if (outcome.result?.error) throw outcome.result.error;
      return outcome.exitCode ?? 1;
    }
    const result = command === 'install' ? installDesktop(options) : desktopStatus(options);
    process.stdout.write(parsed.options.json ? `${JSON.stringify(result, null, 2)}\n`
      : command === 'install' ? `Desktop installed: ${result.appPath}\nLaunch: jarvos desktop --workspace ${JSON.stringify(options.workspace)}\n`
        : result.ready ? `Desktop ready: ${result.appPath}\n` : `Desktop unavailable: ${result.problem}\n`);
    return command === 'status' && !result.ready ? 1 : 0;
  } catch (error) {
    process.stderr.write(`jarvos desktop failed: ${error.message}\n`);
    return 1;
  }
}
...
async function runCli(argv = process.argv.slice(2), env = process.env, invokedAs = process.argv[1]) {
  ...
  if (parsed.command === 'help'
    || parsed.help && !['doctor', 'init', 'sync', 'desktop'].includes(parsed.command)) {
  ...
  if (parsed.command === 'desktop') return runDesktop(parsed, env);
  ...
  if (parsed.command === 'doctor') {
    ...
    try {
      if (['local-openclaw', 'v0-5-0'].includes(parsed.options.profile)) {
        ...
        let report = await profileDoctor.runProfileDoctor({
        ...
        const modules = loadHealthModules({ workspace: report.workspace, profile: report.profile }).modules;
        const systemBlocked = modules.some((module) => (
          module.id === 'system' && healthModuleBlocksDoctor(module)
        ));
        report = attachSystemDoctorReceipt({ ...report, ok: report.ok && !systemBlocked, modules });
