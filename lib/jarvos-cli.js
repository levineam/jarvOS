...
function checkControlPlaneModule() {
  ...
  try {
    ...
    const hostReady = typeof manager.verifyHostService === 'function'
      && manager.verifyHostService(process.env.JARVOS_CONTROL_PLANE_SERVICE_MODULE).ok;
    const ok = typeof manager.createControlPlaneService === 'function'
      && typeof context.controlPlane === 'function'
      && compatible
      && dependency
      && hostReady;
    return result(
      ...
      ok
        ...
        : 'control plane is not ready: configure a usable JARVOS_CONTROL_PLANE_SERVICE_MODULE (doctor does not print its value)',
    );
  } catch (error) {
