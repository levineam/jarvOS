...
function checkJournalConflict(configPath) {
  ...
  // automation — the community "journals" plugin or the core "daily-notes" plugin — is
  // enabled and writes into the same folder jarvOS uses for journal entries, the two
  // tools fight over the same files and can clobber jarvOS journal content with stubs.
  try {
    const config = readJson(configPath);
    const vaultPath = expandHome(config.vaultPath);
    ...
    if (community.ok && Array.isArray(community.value) && community.value.includes(OBSIDIAN_JOURNAL_PLUGIN)) {
    ...
    if (coreEnabled) {
      ...
      const dailyFolder = folder ? path.resolve(vaultPath, folder) : path.resolve(vaultPath);
      const overlapsJournal = dailyFolder === journalDir
        || dailyFolder === path.resolve(vaultPath)
        || dailyFolder.startsWith(`${journalDir}${path.sep}`)
        || journalDir.startsWith(`${dailyFolder}${path.sep}`);
      if (overlapsJournal) {
