...
function checkVaultPath(configPath) {
  try {
    ...
    if (!vaultPath) {
      return result('vault-path', false, 'vault path', 'paths.vault is not configured; run jarvos sync to migrate this config');
    }
...
function checkVaultPathStale(configPath) {
  ...
  try {
    ...
    const vaultPath = configuredVaultPath(config);
    if (!vaultPath) {
      return result('vault-path-stale', false, 'vault path freshness', 'paths.vault is not configured; run jarvos sync to migrate this config');
    }
