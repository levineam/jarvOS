...
function validateConfigShape(config, schema = loadConfigSchema()) {
  return validateConfigValue(config, schema);
}

function configuredVaultPath(config) {
  const configured = config?.paths?.vault || config?.vaultPath;
...
function checkVaultPath(configPath) {
  try {
    const config = readJson(configPath);
    const vaultPath = configuredVaultPath(config);
    if (!vaultPath) {
      return result('vault-path', false, 'vault path', 'paths.vault or vaultPath is not configured');
    }
...
function checkVaultPathStale(configPath) {
  ...
  try {
    ...
    const vaultPath = configuredVaultPath(config);
    if (!vaultPath) {
      return result('vault-path-stale', false, 'vault path freshness', 'paths.vault or vaultPath is not configured');
    }
...
jarvOS vault. The command validates Notes/, Journal/, and Tags/ but never writes
inside the vault. Use --dry-run first. Applying the plan refuses to replace a
different existing config; an identical config is treated as already synced.
...
function renderSyncResult(payload) {
  const lines = [
    ...
    `Mode: ${payload.dryRun ? 'DRY RUN' : (payload.changed ? 'APPLIED' : 'ALREADY SYNCED')}`,
  ...
  lines.push(`Next: jarvos doctor --profile minimal --workspace ${JSON.stringify(payload.workspace)}`);
  return lines.join('\n');
}

function runSync(argv = [], env = process.env) {
  ...
  try {
    ...
    if (!parsed.options.name) {
      throw new Error('--name is required so the shared identity is not guessed');
    }
    if (!parsed.options.timezone) {
      throw new Error('--timezone is required so journal dates are not guessed');
    }
    ...
    const vaultDir = parsed.options.vault || discoverExistingVault({ homeDir });
    if (!vaultDir) {
      throw new Error('No existing jarvOS vault was found; pass --vault explicitly');
    }
    const user = { name: parsed.options.name, timezone: parsed.options.timezone };
    ...
    const targetAssessment = assessSharedVaultConfigTarget({ configPath, config, homeDir });
    ...
    if (!parsed.options.dryRun) {
      changed = writeSharedVaultConfig({
        configPath,
        vaultDir,
    ...
    const payload = {
      ok: true,
      action: parsed.options.dryRun ? 'dry-run' : (changed ? 'applied' : 'already-synced'),
      dryRun: Boolean(parsed.options.dryRun),
      changed,
      targetAction: targetAssessment.action,
      workspace,
