...
- [`v0.11.0`](https://github.com/levineam/jarvOS/releases/tag/jarvos-bootstrap-v0.11.0)
  is the current public preview release for `jarvos-bootstrap`.
  ...
  changelog.
- Post-v0.11.0 work belongs to one versionless forming batch. The batch has no
...
```bash
npm run release:status -- --version 0.11.0 --source-ref origin/main --verify
```

The example above checks the published baseline; observing an open candidate
instead uses the version actually proposed by Release Please, not a preselected
next target.
