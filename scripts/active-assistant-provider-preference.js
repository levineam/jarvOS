...
} = require('../modules/jarvos-runtime-kit/src/provider-preference.js');

function usage() {
  return [
    'Usage:',
    '  active-assistant-provider-preference.js catalog [--json]',
    '  active-assistant-provider-preference.js status [--json]',
    '  active-assistant-provider-preference.js propose <entry-id> [--json]',
    '  active-assistant-provider-preference.js preview <entry-id> --result <passed|failed> [--json]',
  ].join('\n');
}
...
if (require.main === module) {
  try {
    ...
    process.stderr.write(`active-assistant-provider-preference: ${error.message || String(error)}\n`);
    process.exitCode = 2;
  }
}
