...
const { ACTIVE_ASSISTANT_SEMANTIC_POLICY_VERSION, composeSegments } = require('./active-assistant-semantic-policy');
...
function validateProposal(proposal) {
  if (!proposal || typeof proposal !== 'object' || Array.isArray(proposal) || proposal.contractVersion !== ACTIVE_ASSISTANT_SYNTHESIS_CONTRACT_VERSION) return { ok: false, reasonCode: 'malformed_envelope' };
  ...
  if (proposal.kind !== 'proposal' || !Array.isArray(proposal.segments) || Object.prototype.hasOwnProperty.call(proposal, 'sourcePacketDispositions')) return { ok: false, reasonCode: 'malformed_envelope' };
...
function composeProposal({ proposal, eligibleSourceIds = [], greeting = 'Good morning, Sir!', maxLength = 2000 } = {}) {
  ...
  if (validation.value.kind === 'no_nudge') {
    ...
    return { ok: true, terminalOutcome: 'intentional_silence', reasonCode: 'intentional_no_nudge', accepted: [], rejected: [], message: null, dispositions: deriveDispositions({ eligibleSourceIds: eligible, retainedSourceRefs: refs }) };
  }
...
function redactedReceipt({ runId, terminalOutcome, evidenceDigest, artifactDigest = null, accepted = [], rejected = [] } = {}) {
  if (!TERMINAL_OUTCOMES.includes(terminalOutcome)) return { ok: false, reasonCode: 'unknown_terminal_outcome' };
  return {
    schemaVersion: 1,
    contractVersion: ACTIVE_ASSISTANT_SYNTHESIS_CONTRACT_VERSION,
    policyVersion: ACTIVE_ASSISTANT_SEMANTIC_POLICY_VERSION,
    runId: String(runId || ''),
    terminalOutcome,
...
module.exports = { ACTIVE_ASSISTANT_SYNTHESIS_CONTRACT_VERSION, TERMINAL_OUTCOMES, validateProposal, composeProposal, deriveDispositions, redactedReceipt };
