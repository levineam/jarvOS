...
If this workspace has a configured GBrain provider, use it for brain-native
lookup and writeback: people, companies, concepts, beliefs, predictions,
ontology entries, and reusable skill references. Start with lightweight checks:
...
Do not treat GBrain as the owner of jarvOS governance, project status, or note
capture. Paperclip remains the live task/status authority. `jarvos-secondbrain`
remains the note, journal, provenance, privacy, and QMD pending authority unless
a GBrain path explicitly satisfies the same contract.

Use GBrain skillpacks as scaffold/reference/brain-resident surfaces. Never
silently overwrite local `SKILL.md` files from upstream GBrain content; create a
tracked execution issue and review the diff first.
