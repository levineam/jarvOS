...
function buildObsidianInvariantProgram(operation, inspectionToken, inspectionNonce, expectedVaultRoot) {
  const encoded = payload({ ...operation, inspectionToken, inspectionNonce, expectedVaultRoot });
  return `(() => { const input = JSON.parse(new TextDecoder().decode(Uint8Array.from(atob('${encoded}'), c => c.charCodeAt(0)))); const store = globalThis.${RESULT_STORE} ||= {}; const token = input.inspectionToken; const publish = value => { if (store[token]?.nonce === input.inspectionNonce) store[token] = { ...value, nonce: input.inspectionNonce }; }; store[token] = { status: 'pending', nonce: input.inspectionNonce }; ${vaultIdentityProgram()} if (!correctVault) { publish({ status: 'unavailable' }); return JSON.stringify({ queued: true, token }); } ${invariantTransformProgram()} if (!globalThis.app?.vault?.read || !app.vault.getFileByPath) { publish({ status: 'unavailable' }); return JSON.stringify({ queued: true, token }); } const file = app.vault.getFileByPath(input.vaultRelativePath); if (!file) { const deleted = input.operationKind === 'delete'; publish({ status: deleted ? 'satisfied' : 'missing', invariant: deleted }); return JSON.stringify({ queued: true, token }); } app.vault.read(file).then(content => { const satisfied = input.operationKind === 'delete' ? false : input.operationKind === 'create' || input.operationKind === 'replace' ? content === input.content : transform?.(content) === true; publish({ status: satisfied ? 'satisfied' : 'unsatisfied', invariant: satisfied === true }); }).catch(() => { publish({ status: 'unavailable' }); }); return JSON.stringify({ queued: true, token }); })()`;
}
...
function createVaultMutationAdapter({ vaultRoot, vaultId, vaultName = path.basename(vaultRoot || ''), ledger, ledgerPath, transforms, evaluate, maxPollAttempts = 40, pollIntervalMs = 50, pollTimeoutMs = 2_500, probe, ownerId = crypto.randomUUID(), opportunisticDrain, allowDeleteOperation } = {}) {
  ...
  const expectedVaultRoot = path.resolve(vaultRoot);
  ...
  let draining = false;
  function capability() { if (probe) return probe(); try { const inspected = run(`JSON.stringify({ vaultName: app?.vault?.getName?.(), vaultRoot: app?.vault?.adapter?.getBasePath?.(), hasVault: Boolean(app?.vault?.create && app?.vault?.process && app?.vault?.read) })`); if (!inspected?.hasVault || !inspected.vaultRoot) return { state: 'api_incompatible' }; const actualRoot = path.resolve(inspected.vaultRoot); const sameRoot = process.platform === 'win32' ? actualRoot.toLowerCase() === expectedVaultRoot.toLowerCase() : actualRoot === expectedVaultRoot; return sameRoot && inspected.vaultName === vaultName ? { state: 'available', vaultId } : { state: 'wrong_vault' }; } catch (error) { const message = String(error.message || ''); if (error.code === 'ENOENT') return { state: 'cli_missing' }; if (error.code === 'ETIMEDOUT' || /timed?\s*out|busy|temporar/i.test(message)) return { state: 'app_busy' }; if (/disabled/i.test(message)) return { state: 'cli_disabled' }; if (/unsupported|unknown command|eval/i.test(message)) return { state: 'cli_unsupported' }; if (/not running|no running|connection refused|failed to connect/i.test(message)) return { state: 'app_stopped' }; return { state: 'app_unreachable' }; } }
  function inspectInvariant(input) {
    ...
    try {
      ...
      const queued = run(buildObsidianInvariantProgram(operation, inspectionToken, inspectionNonce, expectedVaultRoot), Math.max(1, deadline - Date.now()));
  ...
  function execute(input, { excludeOperationId } = {}) {
    ...
    try {
      const deadline = Date.now() + pollTimeoutMs;
      const queued = run(buildObsidianMutationProgram(operation, expectedVaultRoot), Math.max(1, deadline - Date.now()));
