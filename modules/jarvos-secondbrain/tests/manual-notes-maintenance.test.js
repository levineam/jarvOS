...
function makeTempVault() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'manual-notes-maintenance-'));
...
test('manual-notes-maintenance dry-run audits notes without writing frontmatter or artifacts', () => {
...
test('manual-notes-maintenance apply normalizes frontmatter and writes optimizer artifacts plus QMD pending record', () => {
  const { root, notesDir, knowledgeDir, statePath } = makeTempVault();
  try {
    const notePath = path.join(notesDir, 'Manual Idea.md');
    fs.writeFileSync(notePath, '# Manual Idea\n\nA useful portable pattern belongs in the stack.\n', 'utf8');
...
test('manual-notes-maintenance preserves sensitive-note local artifacts but skips automatic graph queues', () => {
  const { root, notesDir, knowledgeDir, statePath } = makeTempVault();
  try {
...
test('manual-notes-maintenance since-state catches frontmatter-only privacy changes and cleans queues', () => {
  const { root, notesDir, knowledgeDir, statePath } = makeTempVault();
  try {
    const notePath = path.join(notesDir, 'Manual Idea.md');
    fs.writeFileSync(notePath, '# Manual Idea\n\nA useful portable pattern belongs in the stack.\n', 'utf8');
...
test('manual-notes-maintenance since-state still processes stale audit coverage after dry-run state updates', () => {
  const { root, notesDir, knowledgeDir, statePath } = makeTempVault();
  try {
    const notePath = path.join(notesDir, 'Manual Idea.md');
    fs.writeFileSync(notePath, '# Manual Idea\n\nA useful portable pattern belongs in the stack.\n', 'utf8');
...
test('manual-notes-maintenance apply does not partially rewrite notes with unfixable frontmatter drift', () => {
  const { root, notesDir, knowledgeDir, statePath } = makeTempVault();
  try {
