...
const GENERATED_WIKI_HEADER = 'GENERATED BY jarvos-secondbrain-wiki';
const DEFAULT_MAX_CONCEPT_SOURCES = 25;

function generatedWikiSourceCount(text) {
  const sourcesSection = String(text || '').split(/\n## Sources\n/)[1] || '';
  return (sourcesSection.match(/^- /gm) || []).length;
}

function isGeneratedWikiNavigationDoc(doc, { maxConceptSources = DEFAULT_MAX_CONCEPT_SOURCES } = {}) {
  if (!String(doc.text || '').includes(GENERATED_WIKI_HEADER)) return false;
  if (doc.relativePath === 'index.md') return true;
  if (doc.relativePath.startsWith('concepts/') && generatedWikiSourceCount(doc.text) > maxConceptSources) return true;
  return false;
}

function readMarkdownFiles(rootDir, options = {}) {
  const docs = [];
  ...
  walk(rootDir);
  return docs
    .filter((doc) => !(options.skipGeneratedNavigation && isGeneratedWikiNavigationDoc(doc, options)))
...
function createWikiRetrievalAdapter({ roots = [] } = {}) {
  return {
    ...
    search(question) {
      const docs = roots.flatMap((root) => readMarkdownFiles(root, { skipGeneratedNavigation: true }));
      return searchDocs(question, docs);
    },
  };
}
