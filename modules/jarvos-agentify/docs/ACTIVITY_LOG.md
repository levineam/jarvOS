...
const log = createActivityLog({
  storeDir: '/path/to/store',  // optional; used verbatim when provided
});
```

If `storeDir` is omitted, the adapter uses `JARVOS_AGENTIFY_STORE_DIR`
verbatim when set, otherwise `~/.jarvos/agentify/activity-log`.
...
The `seq` is allocated under a per-tenant append lock before each append. A corrupt or missing `meta.json` falls back to scanning `activity.jsonl`; corrupt JSONL lines are silently skipped on read.
...
|----------|---------|-------------|
| `JARVOS_AGENTIFY_STORE_DIR` | `~/.jarvos/agentify/activity-log` | Root directory for all activity log data, used verbatim when set |
