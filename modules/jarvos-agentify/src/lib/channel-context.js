...
async function buildChannelContext({
  ...
  try {
    ...
    rawMessages = rawMessages.filter(m => m.timestamp >= cutoff);
  } catch (err) {
    errors.push({ source: 'channel_messages', message: publicErrorMessage(err) });
    rawMessages = [];
  }
  ...
  if (guildId) {
    try {
      ...
      errors.push({ source: 'active_threads', message: publicErrorMessage(err) });
      threads = [];
    }
  }
  ...
  if (activityError) {
    errors.push({ source: 'activity_log', message: activityError });
  }
...
function renderContextMarkdown(ctx) {
  ...
  if (Array.isArray(ctx.errors) && ctx.errors.length > 0) {
    ...
    for (const error of ctx.errors) {
      parts.push(`- ${error.source}: ${error.message}`);
    }
  }
