...
function fixture() {
  ...
  for (const dir of [root, stateRoot, worktreeRoot]) { fs.mkdirSync(dir); fs.chmodSync(dir, 0o700); }
...
test('default native runtime provides a bounded plan when the optional provider is unavailable', async () => {
  const f = fixture(); const runtime = createCodexRuntime({ registryPath: f.registryPath });
  const context = runtime.resolveRequest({ repositoryId: 'repo_fixture', subjectKey: 'ORG-PLAN' });
  const inputDigest = 'a'.repeat(64);
  const result = await context.managedWorkflow.plan({ subjectKey: context.subjectKey, canonicalWorktree: context.canonicalWorktree, input: { kind: 'issue', digest: inputDigest } });
  assert.equal(result.ok, true);
  assert.equal(result.route, 'native-fallback');
  assert.equal(result.plan.planDigest, inputDigest);
