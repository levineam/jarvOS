...
if [ "${JARVOS_STEWARDSHIP_ONLY:-0}" = "1" ] && [ "${JARVOS_MANAGED_HARNESS_ROLLBACK:-0}" != "1" ]; then
  ...
  python3 - "$HERMES_CONFIG" "$HOME/.hermes/shell-hooks-allowlist.json" "$JARVOS_STEWARDSHIP_STABLE_ROOT/jarvos-hermes-pre-llm-hook.js" <<'PY'
...
if not os.path.isfile(command) or os.path.islink(command): raise SystemExit('stable Hermes hook is missing or unsafe')
config = {}
...
entries[:] = [item for item in entries if not (isinstance(item, dict) and os.path.basename(str(item.get('command') or '')) in ('jarvos-pre-llm-hook.js', 'jarvos-hermes-pre-llm-hook.js'))]
...
allowlist['approvals'] = [item for item in allowlist['approvals'] if not (isinstance(item, dict) and item.get('event') == 'pre_llm_call' and os.path.basename(str(item.get('command') or '')) in ('jarvos-pre-llm-hook.js', 'jarvos-hermes-pre-llm-hook.js'))]
...
if [ "${JARVOS_MANAGED_HARNESS_ROLLBACK:-0}" = "1" ]; then
  HERMES_CONFIG="${HERMES_CONFIG:-$HOME/.hermes/config.yaml}"
  python3 - "$HERMES_CONFIG" "$HOME/.hermes/shell-hooks-allowlist.json" <<'PY'
import datetime, json, os, shutil, stat, sys, tempfile, yaml
config_path, allowlist_path = sys.argv[1:]
def atomic(path, data, mode):
    if os.path.exists(path):
        with open(path, encoding='utf-8') as source:
            if source.read() == data: return
...
if os.path.exists(config_path):
    ...
    kept = [item for item in entries if not (isinstance(item, dict) and os.path.basename(str(item.get('command') or '')) in ('jarvos-pre-llm-hook.js', 'jarvos-hermes-pre-llm-hook.js'))]
...
if os.path.exists(allowlist_path):
    ...
    if isinstance(approvals, list):
        kept = [item for item in approvals if not (isinstance(item, dict) and item.get('event') == 'pre_llm_call' and os.path.basename(str(item.get('command') or '')) in ('jarvos-pre-llm-hook.js', 'jarvos-hermes-pre-llm-hook.js'))]
